...
def representation(full_cards: Card, player1s: Player, player2s: Player, companion_cards: Card):
    map_house = {'Stark': 1, 'Greyjoy': 2, 'Lannister': 3,
                 'Targaryen': 4, 'Baratheon': 5, 'Tyrell': 6, 'Tully': 7}
    map_companion_cards = {"Jon": 0, "Jaqen": 1,
                           "Gendry": 2, "Melisandre": 3, "Ramsay": 4, "Sandor": 5}
...
def crossover(model1: torch.nn.Module, model2: torch.nn.Module, p_crossover=0.1):
    if random.random() > p_crossover:
        model1_dict = dict([(name, param.data)
                           for (name, param) in model1.named_parameters()])
        model2_dict = dict([(name, param.data)
...
def fittness(model, games=2):
    ...
    parser.add_argument('--player1', metavar='p1', type=str,
                        help="either human or an AI file", default='minimax_agent')
    parser.add_argument('--player2', metavar='p2', type=str,
                        help="either human or an AI file", default='random_agent')
    ...
    parser2 = argparse.ArgumentParser(
        description="A Game of Thrones: Hand of the King")
    parser2.add_argument('--player1', metavar='p1', type=str,
                         help="either human or an AI file", default='random_agent')
    parser2.add_argument('--player2', metavar='p2', type=str,
                         help="either human or an AI file", default='minimax_agent')
    parser2.add_argument('-l', '--load', type=str,
                         help="file containing starting board setup (for repeatability)", default=None)
    parser2.add_argument('-s', '--save', type=str,
                         help="file to save board setup to", default=None)
    parser2.add_argument('-v', '--video', type=str,
                         help="name of the video file to save", default=None)
    games_won = 0
    winner = main.main(parser.parse_args(), model)
    if winner == 1:
        games_won += 1
    else:
        games_won -= 1
    winner = main.main(parser2.parse_args(), model)
    if winner == 1:
        games_won += 1
    else:
        games_won -= 1
...
def selection(population_models, primal_model, next_gen_num=5):
    model_fitness = []
...
def save_weights(model: torch.nn.Module, out_path=".", name="final_model.pt"):
